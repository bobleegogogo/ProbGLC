...
MODELS = {
    ...
    "multidisaster20osm" : {"emb_name" : "street_clip",  "sampler": riemannian_flow_sampler,},
    "multidisaster30osm" : {"emb_name" : "street_clip",  "sampler": riemannian_flow_sampler,},
    "multidisaster20yfcc" : {"emb_name" : "dinov2",  "sampler": riemannian_flow_sampler,},
    "multidisaster30yfcc" : {"emb_name" : "dinov2",  "sampler": riemannian_flow_sampler,},
    "iandisaster20osm" : {"emb_name" : "street_clip", "sampler": riemannian_flow_sampler,},                                                                                                                                                 
    "iandisaster30osm" : {"emb_name" : "street_clip",  "sampler": riemannian_flow_sampler,},                                                                        
    "iandisaster20yfcc" : {"emb_name" : "dinov2",  "sampler": riemannian_flow_sampler,},                                                                                                                                                                                                                                      
    "iandisaster30yfcc" : {"emb_name" : "dinov2",  "sampler": riemannian_flow_sampler,}, 
}
...
def load_prepocessing(model_name, dtype=torch.float32, device="cuda"):
    ...
    if MODELS[model_name]["emb_name"] == "dinov2":
        ...
        return StreetClipFeatureExtractor(device=device)
    else:
...
class PlonkPipeline:
    ...
    def __init__(
        ...
        self.cond_preprocessing = load_prepocessing(model_name=model_path, device=device)
        ...
        if return_traj:
            if num_steps is None:
                output, traj = sampler(
                    ...
                    cfg_rate=cfg,
                    generator=generator,
                    return_trajectories=return_traj,
                )
            else:
                output, traj = sampler(
                    self.model,
            ...
            for i, t in enumerate(traj):
                traj[i] = np.degrees(self.postprocessing(t).detach().cpu().numpy())
            # To degrees
