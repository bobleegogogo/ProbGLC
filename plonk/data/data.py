...
class MultidisasterEarth30(Dataset):
    ...
    def __init__(
        ...
        self.split = split
        self.transforms = transforms
        self.embedding_dir = self.root / embedding_name
        
        # Choose CSV depending on split
        if split == "train":
            csv_path = self.root / "RSI_SVI_UAV_VGI/70%train_RSI_SVI_UAV_VGI.csv"
        elif split == "test":
            csv_path = self.root / "RSI_SVI_UAV_VGI/30%test_RSI_SVI_UAV_VGI.csv"
        else:
            raise ValueError(f"Unknown split: {split}")
        
        # Load file list (no header)
        self.file_df = pd.read_csv(csv_path, header=None, names=["RSI", "SVI", "UAV", "VGI"])
        for col in ["RSI", "SVI", "UAV", "VGI"]:
            self.file_df[col] = self.file_df[col].str.replace("\\", "/", regex=False)
        # Load metadata Excel and create a lookup table
        metadata = pd.read_excel(self.root / "Other/allData_20250320.xlsx")
        self.meta_lookup = metadata.set_index("newnewID")[["Latitude", "Longitude"]]
    ...
    def __getitem__(self, idx):
        ...
        # Extract the number from the VGI filename
        vgi_filename = Path(row["VGI"]).name  # e.g., "1636.jpg"
        newnewID = int(vgi_filename.split(".")[0])
        
        # Get lat/lon from metadata
        lat, lon = self.meta_lookup.loc[newnewID]
        lat = np.radians(lat)
        lon = np.radians(lon)
        
        # Load embedding
        emb_path = self.embedding_dir / f"{newnewID}.npy"
        emb = np.load(emb_path)
        emb = torch.tensor(emb, dtype=torch.float)
        # Package output
        output = {
            "emb": emb,
            "gps": torch.tensor([lat, lon], dtype=torch.float),
            "idx": idx,
            "newnewID": newnewID
        }
...
class MultidisasterEarth20(Dataset):
    ...
    def __init__(
        ...
        # Choose CSV depending on split
        if split == "train":
            csv_path = self.root / "RSI_SVI_UAV_VGI/80%train_RSI_SVI_UAV_VGI.csv"
        elif split == "test":
            csv_path = self.root / "RSI_SVI_UAV_VGI/20%test_RSI_SVI_UAV_VGI.csv"
        else:
            raise ValueError(f"Unknown split: {split}")
...
class BaseIanDisasterEarth(Dataset):

    def __init__(
        self,
        root,
        split="train",
        split_ratio="20",  # "20" or "30"
        embedding_name="VGI_street_embs",
        transforms=None
    ):
        super().__init__()
        self.root = Path(root)
        ...
        # Choose CSV based on split and ratio
        if split_ratio not in ["20", "30"]:
            raise ValueError("split_ratio must be '20' or '30'")
        
        split_name = {"train": f"train_{80 if split_ratio == '20' else 70}_fixed.csv",
                      "test": f"val_{80 if split_ratio == '20' else 70}_fixed.csv"}[split]
        csv_path = self.root / split_name
        self.file_df = pd.read_csv(csv_path, header=None, names=["RSI", "SVI", "VGI"])
        # Drop rows where VGI is missing or empty ("" or NaN)
        #self.file_df = self.file_df[self.file_df["VGI"].notna() & self.file_df["VGI"].str.strip().ne("")]
        for col in ["RSI", "SVI", "VGI"]:
            self.file_df[col] = self.file_df[col].str.replace("\\", "/", regex=False)

        # Metadata
        metadata = pd.read_excel(self.root / "all_20241120.xlsx")
        self.meta_lookup = metadata.set_index("ID")[["Latitude", "Longitude"]]
    def __len__(self):
        return len(self.file_df)
    ...
    def __getitem__(self, idx):
        ...
        # Extract ID from VGI path
        vgi_filename = Path(row["VGI"]).name  # e.g., "1636.jpg"
        id_ = int(vgi_filename.split(".")[0])

        # Get lat/lon
        lat, lon = self.meta_lookup.loc[id_]
        lat = np.radians(lat)
        lon = np.radians(lon)

        # Load embedding
        emb_path = self.embedding_dir / f"{id_}.npy"
        emb = np.load(emb_path)
        emb = torch.tensor(emb, dtype=torch.float)
...
class IanDisasterEarth20(BaseIanDisasterEarth):
    def __init__(self, root, split="train", embedding_name="VGI_street_embs", transforms=None):
        super().__init__(root=root, split=split, split_ratio="20", embedding_name=embedding_name, transforms=transforms)

class IanDisasterEarth30(BaseIanDisasterEarth):
    def __init__(self, root, split="train", embedding_name="VGI_street_embs", transforms=None):
        super().__init__(root=root, split=split, split_ratio="30", embedding_name=embedding_name, transforms=transforms)
